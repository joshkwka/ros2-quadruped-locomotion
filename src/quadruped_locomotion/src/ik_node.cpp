...
public:
    IKNode() : Node("ik_node"), is_standing_(true) {
        publisher_ = this->create_publisher<trajectory_msgs::msg::JointTrajectory>(
            "/leg_controller/joint_trajectory", 10);

        // Timer set to 2 seconds to allow the robot to finish the movement
        timer_ = this->create_wall_timer(
            2000ms, std::bind(&IKNode::publish_trajectory, this));
            
        RCLCPP_INFO(this->get_logger(), "Locomotion Brain Initialized. Commencing pushups!");
    }
...
private:
    void publish_trajectory() {
        ...
        is_standing_ = !is_standing_; // Flip the boolean for the next loop

        // Add a 5cm outward sprawl to the Y-axis so it doesn't tip over
        ...
        // Tell the controller to execute this movement smoothly over 1.0 seconds
